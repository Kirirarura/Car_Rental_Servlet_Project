...
$(function(){
    let numberOfItems = $(".card-content .card-item").length;
    let limitPerPage = 6;
    ...
    function showPage(whichPage){
        ...
        $(".card-content .card-item").hide().slice((currentPage - 1) * limitPerPage, currentPage * limitPerPage).show();

        $(".pagination li").slice(1, -1).remove();

        getPageList(totalPages, currentPage, paginationSize).forEach(item => {
            $("<li>").addClass("page-item").addClass(item ? "current-page" : "dots")
                .toggleClass("active-pagination", item === currentPage).append($("<a>").addClass("page-link")
    ...
    $(document).on("click", ".pagination li.current-page:not(.active-pagination)", function(){
